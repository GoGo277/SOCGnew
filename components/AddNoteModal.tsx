...
import { X, MessageSquare, Send, ShieldCheck } from 'lucide-react';
...
const AddNoteModal: React.FC<AddNoteModalProps> = ({ targetName, targetTypeLabel, isOpen, currentUser, onClose, onSave, language }) => {
        ...
        <form onSubmit={handleSubmit}>
          <div className="flex items-center justify-between p-5 border-b border-zinc-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">{t('add_security_note')}</h3>
          ...
          <div className="p-6 space-y-4">
            <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-xl p-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs font-bold text-zinc-300">{currentUser.username}</p>
                  <p className="text-[10px] text-zinc-500 uppercase tracking-tighter">{currentUser.role} {t('analyst')}</p>
                </div>
              </div>
          ...
          <div className="p-5 bg-zinc-950/50 border-t border-zinc-800 flex justify-end gap-3 rounded-b-2xl">
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-2 bg-amber-600 hover:bg-amber-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-sm font-bold transition-all shadow-lg active:scale-95 uppercase tracking-widest"
            >
