      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#09090b]">
        <header className="h-16 border-b border-zinc-800/50 flex items-center justify-between px-8 bg-[#09090b]/80 backdrop-blur-md z-20 sticky top-0">
          ...
          <div className="flex items-center gap-4">
            <button onClick={handleRefresh} className={`p-2.5 rounded-xl border border-zinc-800 text-zinc-500 hover:text-white transition-all active:scale-95 ${isSyncing ? 'animate-spin border-blue-500/50 text-blue-400' : ''}`} title="Synchronize State"><RefreshCw className="w-5 h-5" /></button>
            <button onClick={onToggleNotifCenter} className={`p-2.5 rounded-xl transition-all relative border ${isNotifCenterOpen ? 'bg-blue-500/10 border-blue-500/50 text-blue-400' : 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-700'}`}><Bell className="w-5 h-5" />{totalAlerts > 0 && (<span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-red-600 text-white text-[9px] font-black rounded-full flex items-center justify-center border-2 border-[#09090b] shadow-lg">{totalAlerts}</span>)}</button>

            <div className="relative">
              <button onClick={() => setIsAccountMenuOpen(!isAccountMenuOpen)} className={`flex items-center gap-3 pl-3 pr-2 py-1.5 rounded-xl border transition-all ${isAccountMenuOpen ? 'bg-zinc-800 border-zinc-600 text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:border-zinc-700'}`}>
